...
def load_data():
    ...
    return {
        ...
        "func_budget": func_budget, "func_ytd": func_ytd,
    ...
    with tab_rev:
        ...
        st.dataframe(
            df_rev.style
                ...
                .background_gradient(subset=["% of Budget"], cmap="Greens", vmin=0, vmax=1),
    ...
    with tab_exp:
        ...
        st.dataframe(
            df_exp[show_cols].style
                ...
                .background_gradient(subset=["% of Budget"], cmap="Oranges", vmin=0, vmax=1),
            use_container_width=True, hide_index=True
        )
...
# ════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        st.dataframe(
            df_pivot.style.format("${:,.0f}").background_gradient(cmap="Greens", axis=1),
...
# ════════════════════════════════════════════════════════════
# PAGE 4 — FUNCTIONAL EXPENSE REPORT
# ════════════════════════════════════════════════════════════
elif page == "🔍 Functional Expense Report":
    st.markdown(f"""<div class="hero">
      <div class="hero-tag">Nonprofit Reporting</div>
      <h1>Functional Expense Report</h1>
      <p>Expenses allocated by functional category using JD allocation percentages</p>
    </div>""", unsafe_allow_html=True)

    section("FY26 Expenses by Functional Category",
            "JD allocation percentages applied — adjust assumptions in tracker tab")

    cats    = list(d["func_budget"].keys())
    budgets = list(d["func_budget"].values())
    ytds    = list(d["func_ytd"].values())
    total_b = sum(budgets)
    total_y = sum(ytds)
    ...
    with col1:
        fig_d = go.Figure(go.Pie(
            labels=cats, values=budgets, hole=.55,
            marker_colors=[CAT_COLORS.get(c, SLATE) for c in cats],
            textinfo="label+percent",
            textfont=dict(family="Darker Grotesque", size=11),
            hovertemplate="%{label}<br>$%{value:,.0f}<extra></extra>",
        ))
        fig_d.update_layout(
            title="FY26 Budget Allocation",
            showlegend=False, paper_bgcolor="rgba(0,0,0,0)",
            font=dict(family="Darker Grotesque"),
            margin=dict(l=0,r=0,t=40,b=0), height=300,
            annotations=[dict(
                text=f"${total_b/1000:.0f}K<br><span style='font-size:10px'>Total</span>",
                font=dict(size=14, family="Lora", color=GREEN),
                showarrow=False,
            )]
        )
    ...
    with col2:
        fig_b = go.Figure()
        for cat, bgt, ytd in zip(cats, budgets, ytds):
            color = CAT_COLORS.get(cat, SLATE)
            fig_b.add_bar(x=[cat], y=[bgt], marker_color=color, opacity=.25,
                          marker_line_color=color, marker_line_width=1.5,
                          showlegend=False)
            fig_b.add_bar(x=[cat], y=[ytd], marker_color=color, showlegend=False)
        fig_b.update_layout(
            barmode="overlay", title="Budget vs. YTD Actual",
            plot_bgcolor="white", paper_bgcolor="rgba(0,0,0,0)",
            font=dict(family="Darker Grotesque", size=11),
            margin=dict(l=0,r=0,t=40,b=70), height=300,
            yaxis=dict(tickprefix="$", tickformat=",.0f", gridcolor="#ece9e1"),
            xaxis=dict(tickangle=-20, gridcolor="rgba(0,0,0,0)"),
        )
    ...
    for cat, bgt, ytd in zip(cats, budgets, ytds):
        rows_f.append({
            ...
            "Remaining": bgt - ytd,
            "% of Budget": ytd/bgt if bgt > 0 else 0,
            "% of Total Budget": bgt/total_b if total_b > 0 else 0,
            "% of Total Actual": ytd/total_y if total_y > 0 else 0,
        })
    rows_f.append({
        "Functional Category": "TOTAL",
        "FY26 Budget": total_b, "YTD Actual": total_y,
        "Remaining": total_b - total_y,
        "% of Budget": total_y/total_b if total_b > 0 else 0,
        "% of Total Budget": 1.0, "% of Total Actual": 1.0 if total_y > 0 else 0,
    })
    ...
    st.dataframe(
        df_func.style
            .format({"FY26 Budget":"${:,.0f}","YTD Actual":"${:,.0f}",
                     "Remaining":"${:,.0f}","% of Budget":"{:.1%}",
                     "% of Total Budget":"{:.1%}","% of Total Actual":"{:.1%}"})
            .apply(lambda x: [
                f"background:{CREAM};font-weight:700"
                if x["Functional Category"]=="TOTAL" else "" for _ in x], axis=1)
            .background_gradient(subset=["% of Total Budget"], cmap="Greens", vmin=0, vmax=1),
        use_container_width=True, hide_index=True
    )

    section("YTD Spend Rate by Category")
    for cat, bgt, ytd in zip(cats, budgets, ytds):
        color = CAT_COLORS.get(cat, SLATE)
        st.markdown(f"**{cat}** — {fmt(ytd)} of {fmt(bgt)}")
        st.markdown(progress_bar(ytd, bgt, color), unsafe_allow_html=True)

    st.markdown(f"<div class='note'>📋 Payroll (5000–5070) allocated per Michele 70/20/10% and Lynnette 40/60/0% (Development/Program/Overhead). Contractor allocations set individually. Adjust percentages in the Functional Expense Report tab of the tracker to update this view automatically.</div>", unsafe_allow_html=True)
